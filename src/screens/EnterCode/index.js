...
import {MatchOtpAPI} from '../../networking/api';
import {MatchOtpPayloads} from '../../networking/payloads';
...
export default class EnterCode extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      code: '',
      isLoading: false,
      draftUserId: this.props.navigation.getParam('draftUserId', ''),
    };
    var t = setInterval(() => {
      this.setState({
  ...
  async onVerifyCode(code) {
    var payload = MatchOtpPayloads(code);
    console.log(payload);
    this.setState({
      isLoading: true,
      code: '',
    });
    console.log(MatchOtpAPI(this.state.draftUserId));
    var responce = await PostRequest(
      payload,
      MatchOtpAPI(this.state.draftUserId),
      true,
    );
    this.setState({
      isLoading: false,
    ...
    if (responce.status == 'success') {
      this.props.navigation.navigate('EnterEmail', {
        draftUserId: this.state.draftUserId,
      });
    } else {
        ...
        <View style={{marginTop: this.state.margin}}>
          {this.state.resendCode == true ? (
            <View
              ...
              <Text
                style={{color: 'white', fontSize: 14, fontFamily: Fonts.Arimo}}>
                Code has been sent
              </Text>
            </View>
          ) : null}
